...
namespace Data.Repository
{
	public class TratamientoData : ITratamientoRepository
	{
		...
		public async Task<bool> EliminarTratamiento(int id)
		{
			...
			{
				SqlCommand cmd = new SqlCommand("uspEliminarTratamiento", conexion);
		...
		public async Task<int> Insertar(Tratamiento data)
		{
			int ultimoId = 0;
			using (SqlConnection conexion = new SqlConnection(cadenaConexion))
			{
		...
		public async Task<List<DTOTratamiento>> ObtenerTratamientoPorFinca(int idFinca)
		{
		...
		public async Task<List<DTOTratamiento>> ObtenerTratamientoPorGanado(string idGanado)
		{
		...
		public async Task<List<DTOTratamiento>> ObtenerTratamientoPorGrupo(int idGrupo)
		{
		...
		public async Task<List<DTOTratamiento>> ObtenerTratamientosPorUsuario(int idUsuario)
		{
		...
		private async Task<List<DTOTratamiento>> ObtenerPor(string procedimientoAlmacenado, SqlParameter parametro)
		{
			List<DTOTratamiento> listaTratamiento = new List<DTOTratamiento>();
			using (SqlConnection conexion = new SqlConnection(cadenaConexion))
			{
				...
				try
				{
					...
					using (SqlDataReader dr = cmd.ExecuteReader())
					{
						...
						{
							listaTratamiento.Add(new DTOTratamiento()
							{
